 ...
 */
export async function generateEnhancedUploadUrl(request, _userAgent) {
    // Validate request
    const validatedRequest = UploadRequestSchema.parse(request);
 ...
 */
export async function processAndUploadImage(buffer, originalName, userAgent, _processingOptions) {
    // Process image with optimization
    const { optimized, thumbnail, originalFormat, optimizedFormat } = await autoOptimizeImage(buffer, originalName, userAgent);
    // Generate filenames
 ...
 */
export async function bulkUploadImages(files, userAgent, processingOptions) {
    const results = [];
    for (const file of files) {
        try {
            const result = await processAndUploadImage(file.buffer, file.originalName, userAgent, processingOptions);
            results.push(result);
        }
        catch (error) {
