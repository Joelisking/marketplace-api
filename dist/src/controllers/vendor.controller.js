...
export async function getVendorDashboard(req, res) {
    ...
    const [totalProducts, lowStockProducts, visibleProducts, totalOrders, totalRevenue, totalItemsSold, recentOrders,] = await Promise.all([
...
export async function getVendorProductStats(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
    ...
    const [totalProducts, visibleProducts, hiddenProducts, lowStockProducts, outOfStockProducts, totalValue, totalSold, totalRevenue,] = await Promise.all([
        prisma.product.count({
            where: { storeId: store.id },
        }),
        prisma.product.count({
...
export async function getVendorBestSellers(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
...
export async function getVendorStore(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
