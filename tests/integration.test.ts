...
describe('Marketplace Integration Tests', () => {
  let vendor1: TestUser;
  let vendor2: TestUser;
  let customer: TestUser;
  let store1: TestStore;
  let store2: TestStore;
  let products1: TestProduct[];
  let products2: TestProduct[];

  const testData = generateTestData();

  beforeAll(async () => {
    // Create test users
    vendor1 = await createTestUser(testData.vendorEmail, 'CUSTOMER');
    const vendor2Email = `vendor2-${Date.now()}-${Math.random().toString(36).substr(2, 9)}@test.com`;
    vendor2 = await createTestUser(vendor2Email, 'CUSTOMER');
    customer = await createTestUser(testData.customerEmail, 'CUSTOMER');

    // Create stores for vendors (this will promote them to VENDOR role)
    store1 = await createTestStore(vendor1.token, {
      name: testData.storeName,
      slug: testData.storeSlug,
      logoUrl: 'https://example.com/logo1.jpg',
    });

    store2 = await createTestStore(vendor2.token, {
  ...
  afterAll(async () => {
    await cleanupTestData([vendor1.id, vendor2.id, customer.id], [store1.id, store2.id]);
  });

  describe('Complete Marketplace Workflow', () => {
    it('should allow vendors to manage their stores and products independently', async () => {
      // Vendor 1 should only see their own products (new role-based filtering)
      const vendor1ProductsResponse = await request(app)
        .get('/products')
      ...
      const vendor2ProductsResponse = await request(app)
        .get('/products')
      ...
      const customerProductsResponse = await request(app)
        .get('/products')
      ...
      const storeIds = customerProductsResponse.body.items.map((p: any) => p.storeId);
      expect(storeIds).toContain(store1.id);
      expect(storeIds).toContain(store2.id);
    });

    it('should allow customers to browse all products', async () => {
      ...
      // Should include products from both stores
      const storeIds = response.body.items.map((p: any) => p.storeId);
      expect(storeIds).toContain(store1.id);
      expect(storeIds).toContain(store2.id);
    });

    it('should allow searching and filtering products', async () => {
      // Search by product name - use the actual product name created in setup
      const searchResponse = await request(app).get(`/products?q=${products1[0].name}`);
      ...
      // Filter by store
      const storeFilterResponse = await request(app).get(`/products?storeId=${store1.id}`);

      expect(storeFilterResponse.status).toBe(200);
      expect(storeFilterResponse.body.items.every((p: any) => p.storeId === store1.id)).toBe(true);
    });

    it('should allow browsing stores and their products', async () => {
      // List all stores
      ...
      // Get specific store
      const storeResponse = await request(app).get(`/stores/${store1.slug}`);
      ...
      const storeProductsResponse = await request(app).get(`/stores/${store1.slug}/products`);

      expect(storeProductsResponse.status).toBe(200);
      expect(storeProductsResponse.body.items).toHaveLength(2);
      expect(storeProductsResponse.body.items.every((p: any) => p.storeId === store1.id)).toBe(
        true,
      );
    });
  });
  ...
  describe('Authorization and Security', () => {
    it("should prevent vendors from accessing other vendors' resources", async () => {
    ...
    it('should prevent customers from creating stores or products', async () => {
      ...
      expect(productResponse.body.message).toBe(
    ...
    it('should require authentication for write operations', async () => {
      ...
      const storeResponse = await request(app).post('/stores').send({
  ...
  describe('Pagination and Performance', () => {
    it('should handle pagination correctly', async () => {
      ...
      const paginationProducts = await Promise.all(
      ...
      // Test first page
      const page1Response = await request(app).get('/products?page=1&limit=2');
      ...
      // Test second page
      const page2Response = await request(app).get('/products?page=2&limit=2');
