...
export interface TestStore {
  ...
  slug: string;
  name: string;
  newToken?: string; // Optional new token with updated role
}
...
export async function createTestStore(
  vendorToken: string,
  storeData: { name: string; slug: string; logoUrl?: string },
  userEmail?: string,
): Promise<TestStore> {
 ...
 * Clean up test data
 */
export async function cleanupTestData(userIds: string[], storeIds: string[]) {
 ...
 */
export async function verifyResourceNotExists(
  endpoint: string,
  expectedStatus = 404,
): Promise<boolean> {
  try {
