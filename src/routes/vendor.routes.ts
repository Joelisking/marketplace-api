...
import { z } from 'zod';
import {
  createVendorPaystackAccount,
  updateVendorPaystackAccount,
  getVendorPaystackAccount,
  listVendorPaystackAccounts,
...
r.put(
  ...
  authGuard,
  requireVendor,
  vendorAnalytics.updateFulfillmentTrackingController,
);
r.get(
  '/orders/:orderId/tracking',
  authGuard,
  requireVendor,
  vendorAnalytics.getFulfillmentTrackingController,
);
r.post(
  '/orders/:orderId/picked-up',
  authGuard,
  requireVendor,
  vendorAnalytics.markOrderPickedUpController,
);
r.post(
  '/orders/:orderId/out-for-delivery',
  authGuard,
  requireVendor,
  vendorAnalytics.markOrderOutForDeliveryController,
);
r.post(
  '/orders/:orderId/delivered',
  authGuard,
  requireVendor,
  vendorAnalytics.markOrderDeliveredController,
);
...
r.post('/payment/account', authGuard, requireVendor, async (req, res) => {
  try {
    ...
    if (data.vendorId !== (req as any).user.id) {
